...
<div class="pagenation">
    ...
    <c:choose>
        <c:when test="${currentPage > 1}">
            <a href="${finalPageUrl}?page=0&size=10">
    ...
    <c:choose>
        <c:when test="${currentPage > 1}">
            <a href="${finalPageUrl}?page=${currentPage - 2}&size=10">
    ...
    <c:forEach var="i" begin="${startPage}" end="${endPage}">
        <c:choose>
            ...
            <c:otherwise>
                <a href="${finalPageUrl}?page=${i - 1}&size=10">${i}</a>
            </c:otherwise>
        </c:choose>
    </c:forEach>
    ...
    <c:choose>
        <c:when test="${currentPage < totalPage}">
            <a href="${finalPageUrl}?page=${currentPage}&size=10">
    ...
    <c:choose>
        <c:when test="${currentPage < totalPage}">
            <a href="${finalPageUrl}?page=${totalPage - 1}&size=10">
