...
<script>
	...
	const loginUserEName = "${eName}";
	const userAuth = "${loginUser.uAuth}";
	let currentType = null;
	let selectedUsers = {};
	let removedUsers = new Set();
	console.log("loginUserEName: " + loginUserEName);
	...
	function ensureSelfSelected() {
	    ...
	    if ((userAuth === "BOARD_MANAGER" || userAuth === "USER") && ifSelfMissing) {
	        selectedUsers[String(loginUserNum)] = loginUserEName;
	        updateSelectedUserDisplay();
	    }
	}
	...
	document.getElementById("boardForm").addEventListener("submit", function (e) {
	    ...
	    if (hasForbiddenWord) {
	        alert("‘공지’, ‘관리자’가 포함된 게시판명은 추가할 수 없습니다.");
	        return false;
	    }
	...
	function renderMemberList(members) {
	    ...
	    members.forEach(member => {
	        ...
	        if(userAuth === "USER" && ifSelf) {
	        	...
	        	selectedUsers[String(member.uNum)] = loginUserEName;
