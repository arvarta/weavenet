...
<body>
    <form id="writeForm">
        ...
        <div id="button" class="btn_top">
            <span class="btnBc outline medium">
                <input type="button" value="취소" onclick="location.href='/api/admin/posts/${p.pNum}';" /> 
            </span>
                ...
                <c:forEach var="b" items="${boards}" varStatus="status">
                    <c:if test="${not empty b.bNum}">
                        ...
                        <c:if test="${isManager or isMyDeptBoard or isGeneralBoard}">
                            <li>
                                <c:set var="inputId" value="board_edit_${b.bNum}" />
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            // 새 파일 입력 변경 JS (DataTransfer 사용)
            $('#newFileInput').on('change', function() {
                const files = this.files;
                for(let i=0; i < files.length; i++){
                    let exists = false;
                    for(let j=0; j < dataTransfer.files.length; j++){
                        if(dataTransfer.files[j].name === files[i].name){
                            exists = true; break;
                        }
                    }
                    if(!exists){ dataTransfer.items.add(files[i]); }
                }
            ...
            $('#newFileListDisplay').on('click', '.delete-new-file-btn', function() {
                ...
                for (let i = 0; i < dataTransfer.files.length; i++) {
                    if (i !== indexToRemove) { newFiles.items.add(dataTransfer.files[i]); }
                }
            ...
            $('#btnConfirm').click(function () {
                ...
                fetch(`${pageContext.request.contextPath}/api/admin/posts/${pNum}/update`, {
                ...
                .then(result => {
                    if (result === "success") {
                        alert("게시글이 수정되었습니다.");
                        location.href = `${pageContext.request.contextPath}/api/admin/posts/${pNum}`; 
                    } else {
        ...
        function selectBoard(bNum, bTitle) {
             $('#bNumInput').val(bNum);
             $('#selectedBoardText').text(bTitle);
             $('#boardSelectModal').removeClass('show');
        }
